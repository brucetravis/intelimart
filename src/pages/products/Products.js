...
export default function Products() {
  ...
  }, [searchTerm, products]) // watch out for the search term and the products
              ...
              {!loading && filteredProducts.length > 0 ? (
                filteredProducts?.map((product) => (
                  <tr key={product._id}>
                    ...
                    <td>
                      {
                        new Date(product.updatedAt).toLocaleString().length > 10 ? new Date(product.updatedAt).toLocaleString().substring(0, 10) 
